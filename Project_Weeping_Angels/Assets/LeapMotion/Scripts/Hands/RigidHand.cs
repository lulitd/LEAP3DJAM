...
public class RigidHand : SkeletalHand {
	...
	void Update(){
		...
		Debug.Log ("Weapon index " + weaponValue.weaponIndex);

		if (weaponValue.weaponIndex == 1 && stick != null) {
			stick.transform.position = GetPalmCenter();
			
		} 
		if (weaponValue.weaponIndex == 2 && rock != null) {
			rock.transform.position = GetPalmCenter();

		} 
		if (weaponValue.weaponIndex == 3 && chickenLeg != null) {
			chickenLeg.transform.position = GetPalmCenter ();
		}
		if (weaponValue.weaponIndex == 4 && shieldInHand != null) {
